                        ...
                        <tbody>
                            @foreach ($surveys as $survey)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('surveys.show', $survey->id) }}" class="btn btn-sm btn-info">Ver</a>
                                        <a href="{{ route('surveys.edit', $survey->id) }}" class="btn btn-sm btn-warning">Editar</a>
        ...
        $(document).ready(function() {
            ...
            $('#createSurveyBtn').on('click', function() {
                Swal.fire({
                    ...
                    html:
                    '<input id="service_employee_id" class="swal2-input" placeholder="ID Empleado Servicio">' +
                    '<input id="counter_employee_id" class="swal2-input" placeholder="ID Empleado Mostrador">' +
                    '<input id="customer_id" class="swal2-input" placeholder="ID Cliente">' +
                    '<input id="service_rating" class="swal2-input" type="number" placeholder="Calificación Servicio" min="1" max="5">' +
                    '<input id="counter_rating" class="swal2-input" type="number" placeholder="Calificación Mostrador" min="1" max="5">' +
                    '<textarea id="service_feedback" class="swal2-textarea" placeholder="Comentarios Servicio"></textarea>' +
                    '<textarea id="counter_feedback" class="swal2-textarea" placeholder="Comentarios Mostrador"></textarea>',
                    focusConfirm: false,
                    preConfirm: () => {
                        let data = {
                            service_employee_id: $('#service_employee_id').val(),
                            counter_employee_id: $('#counter_employee_id').val(),
                            customer_id: $('#customer_id').val(),
                            service_rating: $('#service_rating').val(),
                            counter_rating: $('#counter_rating').val(),
                            service_feedback: $('#service_feedback').val(),
                            counter_feedback: $('#counter_feedback').val(),
                            _token: '{{ csrf_token() }}'
                        };
                        $.post('{{ route('surveys.store') }}', data, function(response) {
                            Swal.fire('Guardado!', 'La encuesta ha sido guardada.', 'success').then(() => {
                                location.reload();
                            });
                        }).fail(function() {
                            Swal.fire('Error!', 'Hubo un problema al guardar la encuesta.', 'error');
                        });
                    }
                });
            });
